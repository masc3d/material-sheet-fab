...
class ImportService : org.deku.leoz.service.internal.ImportService {
    ...
    override fun import(scanCode: String, stationNo: Int): String {
        ...
        var title = ""//""Ok"
        if (unitRecord.dteingangdepot2 != null && unitRecord.dteingangdepot2 == Date().toDateWithoutTime().toTimestamp()) {
            title = ImportService.ResponseMsg.PARCEL_ALREADY_SCANNED.value //"Parcel already scanned"
            return mapper.writeValueAsString(title)
        }
        unitRecord.dteingangdepot2 = Date().toDateWithoutTime().toTimestamp()
