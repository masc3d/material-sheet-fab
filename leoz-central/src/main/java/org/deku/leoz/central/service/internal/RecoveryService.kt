...
class RecoveryService : org.deku.leoz.service.internal.RecoveryService {
    ...
    override fun recoverMobileParcelMessages(dryRun: Boolean, logDate: ShortDate?, applicationUpdateVersion: String?) {
        val mobileLogDir = storage.logDirectory.resolve(BundleType.LEOZ_MOBILE.value)

        val reFilename = Regex("^leoz-mobile-([0-9a-f]+)${if (logDate != null) "-${logDate}" else ""}.log$")
        ...
        val updateLogEntryRegex = Regex("^.*\\[ApplicationUpdate\\(.*version=$applicationUpdateVersion.*\\)\\]$", RegexOption.DOT_MATCHES_ALL)

        logFiles.forEach { file ->
            log.info { "Scanning ${file} for parcel messages" }
            var validNodeVersion = applicationUpdateVersion.isNullOrEmpty()
            ...
            file.bufferedReader().use { reader ->
                while (true) {
                    ...
                    if (!validNodeVersion && line.indexOf("[ApplicationUpdate") >= 0) {
                        validNodeVersion = line.matches(updateLogEntryRegex)
                    }

                    if (validNodeVersion && line.indexOf("[ParcelMessage") >= 0) {
