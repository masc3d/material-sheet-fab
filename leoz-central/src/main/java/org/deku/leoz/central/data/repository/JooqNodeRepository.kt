...
fun SelectWhereStep<MstNodeRecord>.fetchByUid(nodeUid: String, strict: Boolean = true): MstNodeRecord? {
    val nodeUidBytes = nodeUid.replace("-", "").parseHex()

    return when {
        strict -> {
            // Only allow precise matches
            this.where(MST_NODE.UID.eq(nodeUidBytes)).fetchOne()
        }
        else -> {
            // Allow short/truncated uids
            this.where(MST_NODE.UID.startsWith(nodeUidBytes)).toList().let {
                when {
                    it.count() > 1 -> null
                    else -> it.firstOrNull()
                }
...
fun SelectWhereStep<MstNodeRecord>.fetchIdByUid(uid: String):Int? {
    val nodeUidBytes = UUID.fromString(uid).toByteArray()

    return this.where(MST_NODE.UID.eq(nodeUidBytes))
            .fetchOne(MST_NODE.NODE_ID)
}

fun SelectWhereStep<MstNodeRecord>.fetchUidById(id: Int): UUID? {
    return this.where(MST_NODE.NODE_ID.eq(id))
            .fetchOne(MST_NODE.UID)
            ?.toUUID()
}
