        ...
        val module = Kodein.Module {
            ...
            bind<AidcReader>() with singleton {
                ...
                aidcReaders.firstOrNull { it is HoneywellAidcReader }?.also {
                    // Bind exclusively to avoid honeywell issue where claiming the reader may fail sporadically
                    // it.bindExclusively = true

                    // TODO: apparently honeywell may close down adic reader by itself without being released (when wg. application suspends to service)
                    // TODO: check aidc manager device listener debug logging to possibly mitigate this (by explicitly unbinding on disconnect)
                }
