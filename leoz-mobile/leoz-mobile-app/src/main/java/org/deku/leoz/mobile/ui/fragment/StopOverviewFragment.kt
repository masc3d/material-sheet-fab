...
//        this.uxStopList.adapter = StopListAdapter(context, delivery.stopList.filter { it.state == Stop.State.PENDING })
        ...
        val flexibleAdapter: FlexibleAdapter<IFlexible<*>> = FlexibleAdapter(getItemList(delivery.stopList.filter { it.state == Stop.State.PENDING }), this)
