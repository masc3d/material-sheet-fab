...
class UserService
    ...
    override fun get(email: String?, debitorId: Int?): List<UserService.User> {
        ...
        val users = userRecords.mapNotNull { userRec ->
            val authorized = (authorizedUser == null || authorizedUser.role == UserRole.ADMIN || (
        ...
        emf.transaction { em ->
            ...
            if (isNew) {
                ...
                if (user.expiresOn == null) {
                    ...
                    calDate.clear()
                    calDate.set(2099, Calendar.DECEMBER, 31)
                    rec.expiresOn = calDate.toZonedDateTime().toLocalDate().toDate().toSqlDate()
                }

                if (user.passwordExpiresOn == null) {
                    val calDate = GregorianCalendar()
                    calDate.clear()
                    calDate.set(2099, Calendar.DECEMBER, 31)
                    rec.passwordExpiresOn = calDate.toZonedDateTime().toLocalDate().toDate().toSqlDate()
                }
            ...
            if (user.passwordExpiresOn != null)
                rec.passwordExpiresOn = user.passwordExpiresOn
