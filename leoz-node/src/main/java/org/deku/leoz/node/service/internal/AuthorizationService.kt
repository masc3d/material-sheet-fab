...
class AuthorizationService
    ...
    override fun authorizeUser(request: AuthorizationService.Credentials): AuthorizationService.Response {
        ...
        val keyRecord = (if (userRecord.keyUid == null) null else this.keyRepo.findOne(mstKey.uid.eq(userRecord.keyUid))
                .toNullable())
                ?: MstKey().also {
                    it.key = UUID.randomUUID().toString()
                    it.timestamp = Date().toTimestamp()
                    it.type = "user"
                    it.uid = UUID.randomUUID()//default-value wird nicht gezogen

                    keyRepo.save(it)

                    userRecord.keyUid = it.uid
                }
...
//        val isValid = try { UUID.fromString(keyRecord.key); true } catch(e: Throwable) { false }
    ...
    override fun onMessage(message: AuthorizationService.NodeRequest, replyChannel: sx.mq.MqChannel?) {
        try {
            ...
            var nodeRecord = nodeRepo.findByUid(identityKey.uuid)

            if (nodeRecord == null) {
                // Store new node record
                nodeRecord = MstNode().also {
                    it.uid = identityKey.uuid
