...
class TourServiceV1
    ...
    override fun get(
        ...
        val nodeUidsById =
                this.nodeService.get(
                        id = tourRecords.mapNotNull { it.nodeId?.toInt() }
                )
    ...
    override fun getByNode(nodeUid: String): Tour {
        val nodeId = this.nodeService.getByUid(nodeUid).id

        val tourRecord = tourRepo
                .findOne(tadTour.nodeId.eq(nodeId))
                .toNullable()
                ?: throw NoSuchElementException()
    ...
    fun optimizeForNode(
        ...
        try {
            ...
            val tourRecord = tourRepo.findOne(tadTour.nodeId.eq(node.id))
                    .toNullable()
    ...
    private fun TadTour.toTour(
        ...
        val nodeUid = this.nodeId?.let {
            nodeUid ?: nodeService.get(id = listOf(tourRecord.nodeId.toInt()))
                    .first()
                    .uid
        }
