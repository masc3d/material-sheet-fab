    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            try {
                ...
                log.info("${BundleType.LEOZ_BOOT.value}-${version}")
                ...
                log.info("${BundleType.LEOZ_BOOT.value}-${version}, ${JvmUtil.shortInfoText} pid [${Process.currentProcess.pid}] args ${args.toList().toString()}")
                ...
                // Verify bundle param manually
                if (settings.bundle.isEmpty()) {
                    log.error("Bundle parameter is mandatory")
                    jc.usage()
                    System.exit(-1)
                    return
                }
