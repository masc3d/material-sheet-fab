...
class Duration(val length: Long, val unit: TimeUnit) {
    ...
    operator fun div(divisor: Double): Duration = ofNanos((toNanos() / divisor).toLong())

    operator fun div(divisor: Long): Duration = ofNanos(toNanos() / divisor)
    ...
    companion object {
        ...
        fun ofHours(hours: Long):Duration = Duration(hours, TimeUnit.HOURS)
        fun ofDays(days: Long): Duration = Duration(days, TimeUnit.DAYS)
        fun ofMillis(millis: Long): Duration = Duration(millis, TimeUnit.MILLISECONDS)
    }
}
