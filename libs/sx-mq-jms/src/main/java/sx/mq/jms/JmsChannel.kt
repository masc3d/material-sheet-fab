...
class JmsChannel @JvmOverloads constructor(
     ...
     * @param also Callback for customizing the message before sending
     */
    @JvmOverloads fun send(jmsMessage: Message, also: ((Message) -> Unit)? = null) {
     ...
     * @param also Callback for customizing the message before sending
     */
    fun send(message: Any, also: ((Message) -> Unit)?) {
        this.send(
